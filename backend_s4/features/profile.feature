Feature: Manage User profile data via the RESTful API
  ...
  Scenario: Cannot view another user's profile
    When I send a "GET" request to "/api/profile/2"
    Then the response code should be 403

  Scenario: Can replace their own profile
    When I send a "PUT" request to "/api/profile/1" with body:
      """
      {
        "username": "peter",
  ...
  Scenario: Cannot replace another user's profile
    When I send a "PUT" request to "/api/profile/2" with body:
      """
      {
        "username": "peter",
        "email": "new_email@test.com",
  ...
  Scenario: Can update their own profile
    When I send a "PATCH" request to "/api/profile/1" with body:
      """
      {
  ...
  Scenario: Cannot update another user's profile
    When I send a "PATCH" request to "/api/profile/2" with body:
      """
      {
        "username": "peter",
        "email": "new_email@test.com",
